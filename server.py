...
async def fetch_paper_metadata(client: httpx.AsyncClient, node_id: str, semaphore: asyncio.Semaphore, max_in_edges: int):
...
async def extract_subgraph(req: SolverRequest):
    ...
    async def event_stream():
        try:
            cmd = [bin_path, "--mode", "openalex", "--query", req.query_node, "--k", str(req.k), "--output", out_csv, "--time-limit", str(req.time_limit), "--node-limit", str(req.node_limit), "--max-in-edges", str(req.max_in_edges), "--gap-tol", str(req.gap_tol), "--dinkelbach-iter", str(req.dinkelbach_iter), "--cg-batch-frac", str(req.cg_batch_frac), "--cg-min-batch", str(req.cg_min_batch), "--cg-max-batch", str(req.cg_max_batch), "--tol", str(req.tol)]
            ...
            async with httpx.AsyncClient(headers={"User-Agent": "KDensestGUI"}) as client:
                core_metadata = await asyncio.gather(*[fetch_paper_metadata(client, nid, semaphore, req.max_in_edges) for nid in core_ids])
