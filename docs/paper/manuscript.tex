...
The restricted master is maintained incrementally.  New $x$ variables are added when nodes enter $\A$; new $y$ variables are added when a pending revealed edge has both endpoints active; new $w$ variables are added for pairs involving newly active nodes.  When $\lambda$ changes across Dinkelbach iterations, only the objective coefficient of existing $w$ variables changes.
...
We enforce this condition lazily.  When an integer candidate is produced, we construct a unit-capacity support graph and run a max-flow from $q$ to each selected target $t$.  When the flow falls below $\kappa$, the residual graph identifies a violated cut.  Let $R$ be the source side containing $q$.  We add a cut of the form
\begin{equation}
  \label{eq:kappa-cut}
  \sum_{(u,v)\in \delta(R)} y_{uv} \ge \kappa x_t,
\end{equation}
where $\delta(R)$ denotes the materialized selected-support edges crossing from $R$ to its complement.  We then reject the candidate and reprocess the node with the added inequality.  This is standard lazy separation: we generate connectivity constraints only when needed, avoiding a large static family of cuts.

Note that we write the separation over directed edge variables, while the verifier collapses reciprocal directed arcs into a single undirected support edge.  When a pair $\{u,v\}$ has both $y_{uv}=1$ and $y_{vu}=1$ and the pair crosses the cut, the LP sum in \eqref{eq:kappa-cut} credits two units of capacity while the support verifier credits only one.  The directed cut is therefore looser than the rule it ultimately enforces: it admits some fractional and integer patterns that the verifier later rejects, triggering additional separation rounds.  This observation motivates one of our future directions: explicit undirected support variables $z_{uv}$ would align the cut algebra with the max-flow graph through constraints of the form $\sum_{\{u,v\}\in\delta(R)}z_{uv}\ge\kappa x_t$.
...
On the enumerable test grid, we observe that AvgDeg matches the brute-force average-degree optimum on all $15$ graphs.  BP returns sets whose pair-density value matches brute force in every feasible cell, and it satisfies $|S|\ge k$ throughout.  We observe no solver errors.  We do note two diagnostic flags: at $n=20$, $p=0.5$, seed $4$, $k=3$, $\kappa\in\{1,2\}$, BP returns a $5$-clique whose pair-density value matches the brute-force optimum, and the run records $\texttt{kappa\_verified}=\textrm{True}$ for the returned set; the same run, however, also records $\texttt{kappa\_verify\_failed}=\textrm{True}$, indicating that an earlier greedy pruning step produced a $\kappa$-infeasible candidate and triggered a revert to the unpruned set.  The final returned set therefore satisfies both the objective and the $\kappa$-edge-connectivity requirement; only the audit trail of intermediate states is non-trivial on these two cells.  Aside from those two cells, we interpret this result as a validation of the optimization pipeline: our Dinkelbach updates, active-set formulation, lazy materialization and pricing, branch-and-bound, connectivity separation, triangle cuts, and pruning agree with an independent exhaustive reference on the only instances where the full feasible region is small enough to enumerate.
