...
export default function Sidebar({ width, params, setParams, logs, loading, onExtract, onStop }) {
  ...
  return (
    <div style={{ width: `${width}px` }} className="bg-gray-900 text-white flex flex-col overflow-hidden shrink-0 shadow-xl z-20">
      <div className="p-5 overflow-y-auto flex-grow flex flex-col custom-scrollbar min-w-[200px]">
        <h1 className="text-xl font-bold mb-5 flex items-center gap-2 truncate">
        ...
        <details className="mt-4 border border-gray-700 rounded-lg bg-gray-800/50 group">
          ...
          <div className="p-4 pt-0 space-y-4">
            ...
            <div>
              <label className="block text-gray-400 text-[10px] uppercase mb-1">Max In-Edges (API Limit)</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-gray-400 text-[10px] uppercase mb-1">CG Min Batch</label><input type="number" value={params.cgMinBatch} onChange={set('cgMinBatch')} className="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1 text-white font-mono text-xs" /></div>
              <div><label className="block text-gray-400 text-[10px] uppercase mb-1">CG Max Batch</label><input type="number" value={params.cgMaxBatch} onChange={set('cgMaxBatch')} className="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1 text-white font-mono text-xs" /></div>
            </div>
          </div>
        </details>
        ...
        <div className="mt-4 flex flex-col flex-grow min-h-[180px] border border-gray-700 rounded bg-black shadow-inner overflow-hidden">
          ...
          <div className="p-3 overflow-y-auto font-mono text-[10px] text-green-400 leading-relaxed flex-grow whitespace-nowrap">
      ...
      <div className="p-4 border-t border-gray-800 shrink-0 bg-gray-900 flex gap-2">
        {!loading ? (
          <button onClick={onExtract} className="w-full py-3 rounded font-bold shadow-lg bg-indigo-600 hover:bg-indigo-500 text-white hover:shadow-indigo-500/20">Extract Subgraph</button>
        ) : (
          <>
            ...
            <button onClick={onStop} className="px-5 py-3 rounded font-bold bg-red-600 hover:bg-red-500 text-white shadow-lg flex items-center justify-center"><Square size={16} fill="currentColor"/></button>
          </>
        )}
      </div>
    </div>
